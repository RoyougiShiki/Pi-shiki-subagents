...
const REQUIRED_EXTENSION_FILTERS = ["+src/pi/core/pi.ts", "+src/pi/core/pi-modes.ts"];
...
function loadPackageJson(): JsonObject {
  ...
  if (pkg.name !== PACKAGE_NAME) {
    throw new Error(`Expected package name ${PACKAGE_NAME}, got ${String(pkg.name)}`);
  }
  return pkg;
}
...
function ensureSchema(options: Options): CheckResult {
  if (!options.schema) {
    return { ok: true, changed: false, settingsChanged: false, messages: ["[schema] skipped"] };
  }

  const expected = generatedSchemaJson();
  const current = existsSync(SCHEMA_PATH) ? readFileSync(SCHEMA_PATH, "utf-8") : "";
  const fresh = current === expected;

  if (fresh) {
    return { ok: true, changed: false, settingsChanged: false, messages: ["[schema] up to date"] };
  }
  ...
  if (options.check) {
    return {
      ...
      messages: ["[schema] stale; run bun run pi:sync to update oh-my-opencode-slim.schema.json"],
    };
  }

  writeFileSync(SCHEMA_PATH, expected, "utf-8");
  return { ok: true, changed: true, settingsChanged: false, messages: ["[schema] updated oh-my-opencode-slim.schema.json"] };
}
...
function ensureSettings(options: Options): CheckResult {
  ...
  if (packageIndex < 0) {
    if (!options.write || options.check) {
      return {
        ...
        changed: false,
        settingsChanged: false,
        messages: [`[settings] local package source missing: ${REPO_ROOT} (run bun run pi:sync:write)`],
      };
    }
    packages.push({ source: REPO_ROOT, extensions: [...REQUIRED_EXTENSION_FILTERS] });
  ...
  if (!isObject(entry)) {
    return { ok: false, changed, settingsChanged: changed, messages: ["[settings] local package entry is not an object"] };
  }

  const extensions = Array.isArray(entry.extensions) ? entry.extensions.filter((item): item is string => typeof item === "string") : [];
  const missing = REQUIRED_EXTENSION_FILTERS.filter((filter) => !extensions.includes(filter));
  ...
  if (missing.length > 0) {
    if (!options.write || options.check) {
      return {
        ...
        messages: [`[settings] missing extension filters: ${missing.join(", ")} (run bun run pi:sync:write)`],
      };
    }
    ...
    messages.push("[settings] local package source and extension filters are present");
  }
...
function main(): number {
  ...
  if (ok && options.typecheck) ok = runCommand("typecheck", "bun", ["run", "typecheck"]);
  if (ok && options.test) ok = runCommand("test", "bun", ["test"]);
  if (ok && options.build) ok = runCommand("build", "bun", ["run", "build"]);

  console.log("[info] managed agent files are runtime-generated by ensureAgentFiles().");
  console.log("[hint] Run /reload in the current Pi session to refresh runtime code.");

  const settingsChanged = results.some((result) => result.settingsChanged);
  if (settingsChanged || options.restartHint) {
    console.log("[hint] settings changed; restarting Pi is safer than /reload.");
  }
