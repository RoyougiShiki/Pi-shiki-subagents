...
const ignoredDirs = new Set(['node_modules', '.git', 'dist', 'coverage', '.codebase-memory']);
...
const prodFiles = srcFiles.filter((filePath) => !/\.test\.tsx?$/.test(filePath));
const texts = new Map(srcFiles.map((filePath) => [filePath, read(filePath)]));

const largestFiles = srcFiles
  .map((filePath) => ({ file: rel(filePath), lines: texts.get(filePath).split(/\r?\n/).length }))
...
for (const start of prodFiles) {
  ...
  while (stack.length && cycles.length < 40) {
    ...
    for (const next of prodOutgoing.get(current) ?? []) {
      if (next === start && seen.length > 1) cycles.push([...seen, next].map(rel));
...
for (const [filePath, text] of texts) {
  ...
  if (/src\/pi\/subagent\/subagent-run-(state|view|detail-view|tool-details|widget-lines)\.ts$/.test(file)) {
    if (/@earendil-works\/pi-coding-agent|ctx\.ui|setWidget|renderCall|renderResult/.test(text)) {
      boundaryFindings.push({ file, issue: 'pure subagent module references Pi SDK/UI' });
    }
  }
  if (file === 'src/pi/subagent/subagent-pool.ts') {
    if (/ctx\.ui|setWidget|renderCall|renderResult|from ['"].*widget|from ['"].*renderer/.test(text)) {
      boundaryFindings.push({ file, issue: 'AgentPool/runtime imports or uses UI/renderer' });
    }
  }
}

const allSearchFiles = walk(root).filter((filePath) => /\.(ts|tsx|js|json|md)$/.test(filePath));
...
const docsAndPackageFindings = {
  ...
  readmeMentionsPiModesExtension: read(path.join(root, 'README.md')).includes('./src/pi/core/pi-modes.ts'),
  platformCleanupMentionsOldOpenCodeEntrypoint: read(path.join(root, 'docs/oh-my-opencode-slim/plans/platform-adapter-cleanup/proposal.md')).includes('src/opencode') || read(path.join(root, 'docs/oh-my-opencode-slim/plans/platform-adapter-cleanup/proposal.md')).includes('旧 OpenCode'),
};

const report = {
  summary: {
    srcTypeScriptFiles: srcFiles.length,
    srcTypeScriptLines: [...texts.values()].reduce((total, text) => total + text.split(/\r?\n/).length, 0),
  },
  largestFiles,
  highFanIn,
  highFanOut,
