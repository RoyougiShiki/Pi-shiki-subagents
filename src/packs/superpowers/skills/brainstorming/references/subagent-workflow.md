...
3. **读取三个提示词模板**（见下方"读取模板方式"）
4. **创建 TodoWrite** 追踪任务

## 读取模板方式

**⚠️ CRITICAL:** 必须读取以下三个模板文件：
1. `implementer-prompt.md` - 实现者 subagent 提示词
2. `spec-reviewer-prompt.md` - 规格审查 subagent 提示词
3. `code-quality-reviewer-prompt.md` - 代码质量审查 subagent 提示词

**读取方式：** 优先使用工具直接读取仓库内相对路径；不要硬编码平台目录。

**禁止：** 凭记忆构造提示词，必须使用完整模板。
...
3. 一旦规格审查失败，主 Agent **立即短路**当前轮次并进入修复，不等待质量审查返回。
...
- 创建/更新 TodoWrite
- 派发 `plan_folder` 和 `task_id` 给 subagent
