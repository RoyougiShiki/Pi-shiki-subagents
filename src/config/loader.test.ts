...
describe('loadPluginConfig', () => {
  ...
  test('loads project config from .opencode directory', () => {
    ...
    fs.writeFileSync(
      path.join(projectConfigDir, 'pi-shiki-subagents.json'),
      JSON.stringify({
        agents: {
  ...
  test('loads and merges tool groups from user and project config', () => {
    ...
    fs.writeFileSync(
      path.join(defaultConfigDir, 'pi-shiki-subagents.json'),
      JSON.stringify({
        _tool_groups: {
          shared: ['read'],
          userOnly: ['grep'],
  ...
  test('respects OPENCODE_CONFIG_DIR for user config location', () => {
    ...
    fs.writeFileSync(
      path.join(customDir, 'pi-shiki-subagents.json'),
      JSON.stringify({
...
describe('deepMerge behavior', () => {
  ...
  test('merges nested agent configs from user and project', () => {
    ...
    fs.writeFileSync(
      path.join(userOpencodeDir, 'pi-shiki-subagents.json'),
      JSON.stringify({
        agents: {
...
describe('JSONC config support', () => {
  ...
  test('loads .jsonc file with single-line comments', () => {
    ...
    fs.mkdirSync(projectConfigDir, { recursive: true });
    fs.writeFileSync(
      path.join(projectConfigDir, 'pi-shiki-subagents.jsonc'),
      `{
        // This is a comment
        "agents": {
  ...
  test('loads user config from .jsonc', () => {
    ...
    fs.mkdirSync(userOpencodeDir, { recursive: true });
    fs.writeFileSync(
      path.join(userOpencodeDir, 'pi-shiki-subagents.jsonc'),
      `{
...
describe('loadAgentPrompt', () => {
  ...
  test('loads replacement prompt from {agent}.md', () => {
    const promptsDir = path.join(tempDir, 'opencode', 'pi-shiki-subagents');
  ...
  test('respects OPENCODE_CONFIG_DIR for prompt location', () => {
    ...
    const promptsDir = path.join(customDir, 'pi-shiki-subagents');
    fs.mkdirSync(promptsDir, { recursive: true });
    fs.writeFileSync(
