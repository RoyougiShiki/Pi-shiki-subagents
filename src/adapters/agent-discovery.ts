...
function readAgentFile(filePath: string): AgentConfig | null {
  ...
  const name = frontmatter.name ?? content.match(/^name:\s*(.+)$/m)?.[1]?.trim();
  const description = frontmatter.description ?? content.match(/^description:\s*(.+)$/m)?.[1]?.trim();
...
export function discoverAgents(cwd: string): AgentConfig[] {
  ...
  try {
    if (fs.statSync(directProjectDir).isDirectory()) dirs.push(directProjectDir);
  } catch {}
