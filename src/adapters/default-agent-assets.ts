...
export function getDefaultAgentsPath(): string {
  return firstExistingPath(
    adapterRootCandidates().map((root) => path.join(root, "agents-default.json")),
  );
}
...
export function readDefaultAgentDefinitions(): Record<string, any> {
  try {
    return JSON.parse(fs.readFileSync(getDefaultAgentsPath(), "utf-8")) as Record<string, any>;
  } catch {
    return {};
  }
}
...
export function getDefaultAgentDefinitionNames(): string[] {
  return Object.keys(readDefaultAgentDefinitions())
    .filter((name) => name !== TOOL_GROUPS_CONFIG_KEY);
}
