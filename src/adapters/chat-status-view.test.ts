...
import { createChatStatusView, groupChatStatusViews } from './chat-status-view';
...
describe('chat status view', () => {
  test('formats a workflow agent list row and bottom line', () => {
    ...
    expect(view.listRow).toBe('thinker-analysis · working · 05:12');
    expect(view.bottomLine).toBe('thinker-analysis · working · workflow');
  });
  ...
  test('adds fallback hint for failed or dead agents only in bottom line', () => {
    const failed = createChatStatusView({ name: 'thinker-clarify', state: 'failed', scope: 'workflow' });
    const dead = createChatStatusView({ name: 'worker', state: 'dead', scope: 'workflow' });

    expect(failed.listRow).toBe('thinker-clarify · failed');
    expect(failed.bottomLine).toBe('thinker-clarify · failed · workflow · fallback?');
    expect(dead.bottomLine).toBe('worker · dead · workflow · fallback?');
  });

  test('groups status rows by scope order', () => {
    const workflow = createChatStatusView({ name: 'thinker-analysis', state: 'working', scope: 'workflow' });
    ...
    expect(groups[0].items[0].listRow).toBe('thinker-analysis · working');
