...
const TEST_FAILURE_OUTPUT_PATTERN =
  /\b(?:targeted-tests|tests?):\s*exit=(?!0\b)\d+\b/i;
...
function getCommandText(
  ...
  resolver?: EvidenceAdapterOptions['commandTextResolver'],
): string {
...
function resultText(value: unknown): string {
  ...
  if (Array.isArray(value))
    return value.map(resultText).filter(Boolean).join('\n');
...
function addCommandVerificationKinds(
  kinds: Set<CompletionEvidenceKind>,
  commandText: string,
...
function addGenericVerificationKind(
  kinds: Set<CompletionEvidenceKind>,
  commandText: string,
  suffix: 'success' | 'failure',
): boolean {
...
function addOutputVerificationKinds(
  kinds: Set<CompletionEvidenceKind>,
  text: string,
): boolean {
  let matched = false;
...
function isWaitOnlyInfrastructureCommand(commandText: string): boolean {
  return /^\s*(?:sleep\s+\d+|wait)(?:\s*(?:;|&&)\s*echo\s+[\w -]+)?\s*$/i.test(
    commandText,
  );
}
...
function isInfrastructureFailure(commandText: string, text: string): boolean {
  return (
    isWaitOnlyInfrastructureCommand(commandText) &&
    /Request timed out|MCP error/i.test(text)
  );
}

function canInferSuccessFromCommandText(
  toolName: string,
  evidence: ToolEvidence,
): boolean {
...
export function toCompletionEvidenceSummary(
  ...
  const modificationTools = new Set(
    (options.modificationToolNames ?? [...DEFAULT_MODIFICATION_TOOLS]).map(
      normalizeToolName,
    ),
  );
  const verificationTools = new Set(
    (options.verificationToolNames ?? [...DEFAULT_VERIFICATION_TOOLS]).map(
      normalizeToolName,
    ),
  );
  ...
  for (const evidence of evidences) {
    ...
    if (
      ...
      const semantic = interpretCommandSemantic(
        commandText,
        evidence.exitCode,
        options.commandSemantics,
      );
      if (!semantic.isError) {
    ...
    if (!effectiveSuccess) {
      ...
      addKind(kinds, 'tool_failure');
      lastToolFailureAt = evidence.timestamp;
      const matchedFailure = addCommandVerificationKinds(
        kinds,
        commandText,
        'failure',
      );
      if (matchedFailure && TEST_COMMAND_PATTERN.test(commandText))
        lastTestFailureAt = evidence.timestamp;
      if (matchedFailure && LINT_COMMAND_PATTERN.test(commandText))
        lastLintFailureAt = evidence.timestamp;
      if (matchedFailure && TYPECHECK_COMMAND_PATTERN.test(commandText))
        lastTypecheckFailureAt = evidence.timestamp;
      continue;
    }
    ...
    const canInferCommandSuccess = canInferSuccessFromCommandText(
      toolName,
      evidence,
    );
    let hasVerificationCommand = false;
    if (
      canInferCommandSuccess &&
      TEST_COMMAND_PATTERN.test(commandText) &&
      !outputFailures.test
    ) {
      addKind(kinds, 'test_success');
    ...
    if (
      canInferCommandSuccess &&
      LINT_COMMAND_PATTERN.test(commandText) &&
      !outputFailures.lint
    ) {
      addKind(kinds, 'lint_success');
    ...
    if (
      canInferCommandSuccess &&
      TYPECHECK_COMMAND_PATTERN.test(commandText) &&
      !outputFailures.typecheck
    ) {
      addKind(kinds, 'typecheck_success');
      lastTypecheckSuccessAt = evidence.timestamp;
      hasVerificationCommand = true;
    }
    const hasGenericVerificationCommand =
      canInferCommandSuccess &&
      addGenericVerificationKind(kinds, commandText, 'success');
    const hasOutputVerification = addOutputVerificationKinds(kinds, outputText);
    if (
      hasVerificationTool ||
      hasVerificationCommand ||
      hasGenericVerificationCommand ||
      hasOutputVerification
    ) {
      addKind(kinds, 'verification');
      lastVerificationAt = evidence.timestamp;
      if (TEST_SUCCESS_OUTPUT_PATTERN.test(outputText))
        lastTestSuccessAt = evidence.timestamp;
      if (LINT_SUCCESS_OUTPUT_PATTERN.test(outputText))
        lastLintSuccessAt = evidence.timestamp;
      if (TYPECHECK_SUCCESS_OUTPUT_PATTERN.test(outputText))
        lastTypecheckSuccessAt = evidence.timestamp;
    }
  }

  if (timestampAfter(lastTestSuccessAt, lastTestFailureAt))
    removeKind(kinds, 'test_failure');
  if (timestampAfter(lastLintSuccessAt, lastLintFailureAt))
    removeKind(kinds, 'lint_failure');
  if (timestampAfter(lastTypecheckSuccessAt, lastTypecheckFailureAt))
    removeKind(kinds, 'typecheck_failure');
  if (timestampAfter(lastVerificationAt, lastToolFailureAt))
    removeKind(kinds, 'tool_failure');
  if (
    !['tool_failure', 'test_failure', 'lint_failure', 'typecheck_failure'].some(
      (kind) => kinds.has(kind as CompletionEvidenceKind),
    )
  ) {
    failedToolCount = 0;
  }
  if ((options.pendingSubagentCount ?? 0) > 0)
    addKind(kinds, 'subagent_pending');
...
export function toVerificationEvidenceState(
  ...
  return {
    ...
    hasSubagentPending:
      (summary.pendingSubagentCount ?? 0) > 0 ||
      summary.kinds.includes('subagent_pending'),
  };
}
