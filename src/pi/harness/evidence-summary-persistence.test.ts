...
describe('evidence summary persistence', () => {
  test('saves and loads recovered evidence summary state', async () => {
    await withTempDir(async (dir) => {
      const state = updateRecoveredEvidenceSummaryState(
        createRecoveredEvidenceSummaryState(),
        {
          toolName: 'edit',
          toolCallId: 'edit-1',
      ...
      expect(await saveEvidenceSummaryState(state, dir, 'session-1')).toBe(
        true,
      );
  ...
  test('missing or invalid state returns null', async () => {
    ...
    try {
      await withTempDir(async (dir) => {
        expect(
          await loadEvidenceSummaryState(dir, 'missing-session'),
        ).toBeNull();
        ...
        await writeFile(
          getEvidenceSummaryPath(dir, 'broken-session'),
          'not json',
          'utf8',
        );

        expect(
          await loadEvidenceSummaryState(dir, 'broken-session'),
        ).toBeNull();
