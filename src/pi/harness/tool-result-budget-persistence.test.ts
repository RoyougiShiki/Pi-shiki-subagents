...
import { createToolResultBudgetState, recordReplacement } from "./tool-result-budget-state";
import { getBudgetStatePath, loadBudgetState, saveBudgetState } from "./tool-result-budget-persistence";
  ...
  test("missing or invalid state returns null", async () => {
    ...
    await withTempDir(async (dir) => {
      ...
      await writeFile(statePath, "not json", "utf8").catch(async () => {
        await saveBudgetState(createToolResultBudgetState(), dir, "broken-session");
