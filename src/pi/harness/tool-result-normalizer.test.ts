...
describe('tool-result-normalizer', () => {
  ...
  test('normalizes bash tool result with exit code', () => {
    ...
    expect(result.modelFacingMessage).toEqual({
      type: 'text',
  ...
  test('surfaces semantic message without depending on Pi exit-message formatting', () => {
    const result = normalizeToolResult(
      input({
        ...
        modelFacingContent: {
          type: 'text',
          text: 'pi changed its nonzero-exit wording',
        },
        exitCode: 1,
        isError: true,
      }),
