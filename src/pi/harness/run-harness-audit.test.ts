...
describe('runHarnessAudit', () => {
  test("deduplicates: completion auditor's modification_without_verification supersedes policy's warn", () => {
    ...
    expect(
      result.issues.some(
        (issue) => issue.id === 'modified_without_verification',
      ),
    ).toBe(false);
    expect(
      result.issues.some(
        (issue) => issue.id === 'modification_without_verification',
      ),
    ).toBe(true);
  });
  ...
  test('H5: no longer blocks on test-pass claim text — single rule ignores finalText', () => {
    ...
    expect(
      result.issues.some((issue) => issue.id === 'test_pass_without_evidence'),
    ).toBe(false);
    expect(
      result.issues.some(
        (issue) => issue.id === 'modification_without_verification',
