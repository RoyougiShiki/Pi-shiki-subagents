...
describe("evidence summary state", () => {
  test("records modification and filters stale verification after later modification", () => {
    ...
    const summary = mergeRecoveredCompletionEvidenceSummary({ kinds: [] }, state);
  ...
  test("applies temporal filtering to current evidence windows", () => {
    const summary = toTemporalCompletionEvidenceSummary([
      { toolName: "bash", toolCallId: "test-1", args: { command: "bun test" }, result: "pass", timestamp: 100, success: true },
      { toolName: "edit", toolCallId: "edit-1", args: { path: "file.ts" }, result: "edited", timestamp: 200, success: true },
    ]);
  ...
  test("roundtrips persisted state", () => {
    ...
    expect(fromRecoveredEvidenceSummaryJson(toRecoveredEvidenceSummaryJson(state))).toEqual(state);
    expect(fromRecoveredEvidenceSummaryJson({ version: 2, kinds: [], modifiedFileCount: 0, failedToolCount: 0 })).toBeNull();
  });
});
