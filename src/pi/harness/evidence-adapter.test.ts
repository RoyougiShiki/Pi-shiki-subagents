...
function expectVerification(
  command: string,
  kind: 'test_success' | 'lint_success' | 'typecheck_success',
): void {
  const summary = toCompletionEvidenceSummary([
    evidence({ toolName: 'bash', args: { command }, success: true }),
  ]);
...
describe('evidence adapter', () => {
  ...
  test('maps context-mode code and output to verification kinds', () => {
    const summary = toCompletionEvidenceSummary([
      evidence({
        ...
        result:
          'targeted-tests: exit=0; 75 pass 0 fail\ntypecheck: exit=0; $ tsc --noEmit\ndiff-check: exit=0',
        success: true,
      }),
    ]);
  ...
  test('does not infer verification success from failed context-mode summaries', () => {
    const summary = toCompletionEvidenceSummary([
      evidence({
        toolName: 'context_mode_ctx_execute',
        args: {
          code: "spawnSync('bun', ['test']); spawnSync('bun', ['run', 'typecheck']);",
        },
        result:
          'targeted-tests: exit=1; 3 pass 1 fail\ntypecheck: exit=1\nlint: exit=1',
        success: true,
      }),
    ]);
  ...
  test('keeps completion auditor warning after modification plus ordinary bash', () => {
    ...
    const decision = auditCompletion({
      finalText: '已完成',
      evidence: summary,
    });

    expect(decision.action).toBe('warn');
    expect(decision.issues.map((issue) => issue.id)).toContain(
  ...
  test('uses command semantics to treat grep exit code 1 as success, not tool_failure', () => {
    const summary = toCompletionEvidenceSummary([
      evidence({
        toolName: 'bash',
        args: { command: 'grep pattern file.txt' },
        success: false,
        exitCode: 1,
      }),
    ]);
  ...
  test('keeps quoted fake pipeline as tool failure', () => {
    const summary = toCompletionEvidenceSummary([
      evidence({
        toolName: 'bash',
        args: { command: 'python -c "print(\'| grep pattern\')"' },
        success: false,
        exitCode: 1,
      }),
    ]);
  ...
  test('still treats grep exit code 2 as tool_failure', () => {
    const summary = toCompletionEvidenceSummary([
      evidence({
        toolName: 'bash',
        args: { command: 'grep pattern /nonexistent' },
        success: false,
        exitCode: 2,
      }),
    ]);
  ...
  test('keeps completion auditor warning after modification plus grep no matches', () => {
    const summary = toCompletionEvidenceSummary([
      ...
      evidence({
        toolName: 'bash',
        args: { command: 'grep pattern missing_file' },
        success: false,
        exitCode: 1,
      }),
    ]);
    ...
    expect(decision.issues.map((issue) => issue.id)).not.toContain(
  ...
  test('allows completion auditor after modification plus context-mode verification', () => {
    const summary = toCompletionEvidenceSummary([
      ...
      evidence({
        ...
        result:
          'targeted-tests: exit=0; 51 pass 0 fail\ntypecheck: exit=0; $ tsc --noEmit',
        success: true,
        timestamp: 2,
      }),
    ]);
