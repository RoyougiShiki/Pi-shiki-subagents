...
function mergeKinds(left: readonly CompletionEvidenceKind[], right: readonly CompletionEvidenceKind[]): CompletionEvidenceKind[] {
...
function latestVerificationKindTimestamp(
  ...
  const current = kind === "test_success"
...
function isKindValidAfterModification(
  ...
  if (!VERIFICATION_KINDS.has(kind) || latestModificationAt === undefined) return true;
  if (kind === "test_success") return state.lastTestSuccessAt !== undefined && state.lastTestSuccessAt > latestModificationAt;
  if (kind === "lint_success") return state.lastLintSuccessAt !== undefined && state.lastLintSuccessAt > latestModificationAt;
  if (kind === "typecheck_success") return state.lastTypecheckSuccessAt !== undefined && state.lastTypecheckSuccessAt > latestModificationAt;
  return [state.lastTestSuccessAt, state.lastLintSuccessAt, state.lastTypecheckSuccessAt, state.lastVerificationAt]
    .some((timestamp) => timestamp !== undefined && timestamp > latestModificationAt);
}
...
export function updateRecoveredEvidenceSummaryState(
  ...
  return {
    kinds,
    modifiedFileCount: state.modifiedFileCount + (summary.modifiedFileCount ?? 0),
    ...
    lastVerificationAt: summary.kinds.some((kind) => VERIFICATION_KINDS.has(kind))
      ? maxTimestamp(state.lastVerificationAt, timestamp)
      : state.lastVerificationAt,
    lastTestSuccessAt: latestVerificationKindTimestamp(state, "test_success", timestamp, summary.kinds),
    lastLintSuccessAt: latestVerificationKindTimestamp(state, "lint_success", timestamp, summary.kinds),
    lastTypecheckSuccessAt: latestVerificationKindTimestamp(state, "typecheck_success", timestamp, summary.kinds),
...
function effectiveRecoveredKinds(state: RecoveredEvidenceSummaryState): CompletionEvidenceKind[] {
  return state.kinds.filter((kind) => isKindValidAfterModification(kind, state, state.lastModifiedAt));
}

export function toTemporalCompletionEvidenceSummary(evidences: readonly ToolEvidence[]): CompletionEvidenceSummary {
  const state = evidences.reduce(updateRecoveredEvidenceSummaryState, createRecoveredEvidenceSummaryState());
...
export function mergeRecoveredCompletionEvidenceSummary(
  ...
  return {
    ...
    modifiedFileCount: Math.max(current.modifiedFileCount ?? 0, recovered.modifiedFileCount),
    failedToolCount: Math.max(current.failedToolCount ?? 0, recovered.failedToolCount),
  };
}
...
function isCompletionEvidenceKind(value: unknown): value is CompletionEvidenceKind {
  return typeof value === "string" && new Set<CompletionEvidenceKind>([
    "modification",
    "verification",
    "test_success",
    "test_failure",
...
export function fromRecoveredEvidenceSummaryJson(json: unknown): RecoveredEvidenceSummaryState | null {
  if (!json || typeof json !== "object") return null;
  const record = json as Record<string, unknown>;
  if (record.version !== 1) return null;
  if (!Array.isArray(record.kinds) || !record.kinds.every(isCompletionEvidenceKind)) return null;
  if (typeof record.modifiedFileCount !== "number" || !Number.isFinite(record.modifiedFileCount)) return null;
  if (typeof record.failedToolCount !== "number" || !Number.isFinite(record.failedToolCount)) return null;
  ...
  const lastTypecheckSuccessAt = optionalTimestamp(record.lastTypecheckSuccessAt);
  if (lastModifiedAt === null || lastVerificationAt === null || lastFailureAt === null || lastTestSuccessAt === null || lastLintSuccessAt === null || lastTypecheckSuccessAt === null) return null;
