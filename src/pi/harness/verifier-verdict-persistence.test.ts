...
function verdict(partial: Partial<VerifierVerdictEvidence> = {}): VerifierVerdictEvidence {
...
describe("verifier verdict persistence", () => {
  test("roundtrips minimal verifier verdict state", () => {
    const restored = fromVerifierVerdictsPersistenceJson(toVerifierVerdictsPersistenceJson([
  ...
  test("saves and loads verifier verdicts", async () => {
    await withTempDir(async (dir) => {
      expect(await saveVerifierVerdicts([verdict()], dir, "session-1")).toBe(true);
  ...
  test("missing or invalid verifier state returns null", async () => {
    ...
    try {
      await withTempDir(async (dir) => {
        ...
        await writeFile(getVerifierVerdictsPath(dir, "broken-session"), "not json", "utf8");
