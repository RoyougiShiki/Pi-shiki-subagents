...
describe("completion auditor", () => {
  ...
  test("modification without verification → warn by default", () => {
    ...
    expect(result.issues.map((i) => i.id)).toContain("modification_without_verification");
  ...
  test("modification without verification → block when blockOnUnverifiedModification=true", () => {
    const result = auditCompletion(
      { finalText: "我修改了文件。", evidence: { kinds: ["modification"], modifiedFileCount: 1 } },
  ...
  test("modification with test_success evidence → allow", () => {
    const result = auditCompletion({
      finalText: "修改完成，测试通过。",
      evidence: { kinds: ["modification", "test_success"], modifiedFileCount: 1 },
    });
  ...
  test("modification with verification evidence → allow", () => {
    const result = auditCompletion({
      finalText: "修改完成。",
      evidence: { kinds: ["modification", "verification"], modifiedFileCount: 1 },
    });
    expect(result.action).toBe("allow");
    expect(result.issues).toHaveLength(0);
  });
  ...
  test("no modification evidence → allow regardless of text", () => {
    ...
    const result = auditCompletion({ finalText: "全部完成，测试通过。", evidence: emptyEvidence });
    expect(result.action).toBe("allow");
    expect(result.issues).toHaveLength(0);
  });
  ...
  test("modification with lint_success evidence → allow", () => {
    const result = auditCompletion({
      finalText: "修改完成。",
      evidence: { kinds: ["modification", "lint_success"], modifiedFileCount: 1 },
    });
    expect(result.action).toBe("allow");
  });
  ...
  test("modification with typecheck_success evidence → allow", () => {
    const result = auditCompletion({
      finalText: "修改完成。",
      evidence: { kinds: ["modification", "typecheck_success"], modifiedFileCount: 1 },
    });
    expect(result.action).toBe("allow");
  });
