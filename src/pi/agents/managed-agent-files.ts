...
interface AgentSyncConfig {
  ...
  presets?: Record<
    string,
    Record<string, AgentModelConfig | unknown> | undefined
  >;
...
function withManagedAgentMetadata(content: string): string {
  ...
  const metadata = ['omo-managed: true', `omo-source-hash: ${sourceHash}`];
  const nextBlock = [
    ...block
      .split('\n')
...
function isLegacyGeneratedAgentContent(
  existing: string,
  label: string,
): boolean {
...
function isLegacyOmoAgentContent(
  existing: string,
  sourceContent: string,
  label: string,
): boolean {
  return (
    normalizeAgentContentForComparison(existing) ===
      normalizeAgentContentForComparison(sourceContent) ||
...
function removeStaleManagedAgentFiles(
  agentsDir: string,
  ...
  try {
    ...
    for (const file of files) {
      ...
      const content = fs.readFileSync(
        path.join(defaultAgentsDir, file),
        'utf-8',
      );
