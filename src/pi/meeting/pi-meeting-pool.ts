  ...
  async run(
    ...
    try {
      ...
      for (const p of request.participants) {
        const agentPrompt =
          AGENT_PROMPTS[p.agent]?.prompt || 'You are a specialist.';
      ...
      for (let round = 0; round < maxRounds; round++) {
        const phases: Array<'opening' | 'discussion' | 'final'> = [
          'opening',
          'discussion',
          'final',
        ];
        ...
        if (phase === 'discussion' && previousResponses.size > 0) {
          ...
          for (const [name, prev] of previousResponses) {
            const latest = [...transcript]
              .reverse()
              .find((m) => m.from === name && m.phase === 'discussion');
            if (
              latest &&
              computeSemanticOverlap(prev, latest.content) >= threshold
            ) {
              convergedCount++;
            }
          }
            ...
            if (request.participants.length > 0) {
              ...
              const contextSummary =
                transcript
                  .map(
                    (m) =>
                      `[${m.from}]: ${m.content.length > 300 ? m.content.slice(0, 300) + '...' : m.content}`,
                  )
                  .join('\n') || '(no prior discussion)';
      ...
      // Build result
      const finalPositions = transcript
        .filter((m) => m.round === roundsCompleted - 1)
        ...
        (transcript.length > 0
          ? '\n\n### Discussion\n\n' +
            transcript
              .map(
                (m) =>
                  `**${m.from}** (round ${m.round + 1}/${m.phase}): ${m.content.slice(0, 300)}`,
              )
              .join('\n\n')
          : '');
